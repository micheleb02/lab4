...
TEST_CASE("evolve test") {
  ...
  CHECK(calc_c_m_b_i(flock, b1).x == doctest::Approx(4));

  std::vector<boid> flock_e = evolve_flock(flock, delta_t, s);
  CHECK(flock_e[2].v.v_y == doctest::Approx(2.2));
  CHECK(flock_e[0].v.v_x == doctest::Approx(3.4));
}